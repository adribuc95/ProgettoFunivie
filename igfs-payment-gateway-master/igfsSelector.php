...
class Igfs 
{
	...
	public function processPayment($paymentData = array()){
			...
			$init->errorURL = $this->IgfsErrorURL;
                        $ID_ordine = $paymentData['ID_ordine'];
                        $data = date ("Y/m/d");
			...
			$this->setCookiesValue('payment_id',$payment_id);
                        ...
                        $sql = " INSERT INTO `Pagamento` (`ID_pagamento`, `ID_shop`, `ID_ordine`, `email`, `importo`, `data_pagamento`)"
                        ."VALUES ('$payment_id', '$init->shopID', '$ID_ordine', '$init->shopUserRef',  '$importo_totale', '$data' );";
